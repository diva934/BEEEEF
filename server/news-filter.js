const MIN_SCORE = Math.max(3, Number(process.env.NEWS_MIN_SCORE) || 5);
const MIN_CONTROVERSY_SCORE = Math.max(2, Number(process.env.NEWS_MIN_CONTROVERSY_SCORE) || 3);
...
const CONTROVERSY_KEYWORDS = [
  ...
  /\bmandatory\b/i,
  /\ballowed\b/i,
  /\blegal(?:ize|isation|ization|ly)?\b/i,
  /\billegal\b/i,
  /\bban(?:s|ned)?\b/i,
  /\bblock(?:s|ed)?\b/i,
  /\bregulat(?:e|es|ed|ion)\b/i,
...
const CONFLICT_WORDS = [
  ...
  /\bversus\b/i,
  /\bbacklash\b/i,
  /\bcriticis(?:e|es|ed|ing|m)\b/i,
  /\bcriticiz(?:e|es|ed|ing)\b/i,
  /\bcontrovers(?:y|ial)\b/i,
  /\bdebate(?:s|d)?\b/i,
  /\bdivid(?:e|es|ed|ing)\b/i,
  /\bsplit(?:s|ting)?\b/i,
...
function controversyScoreFromText(title, description = '', category = '') {
  const combined = `${title || ''} ${description || ''}`;
  let score = 0;

  CONTROVERSY_KEYWORDS.forEach(pattern => {
    if (pattern.test(combined)) score += 1;
  });
  CONFLICT_WORDS.forEach(pattern => {
    if (pattern.test(combined)) score += 2;
  });

  if (/^(Should|Must|Could|Can|Will|Would)\b/i.test(String(title || ''))) score += 1;
  if (/\?/.test(String(title || ''))) score += 1;
  if (['politics', 'society', 'economy', 'crypto'].includes(category)) score += 1;
  if (['technology', 'sports'].includes(category) && /\b(should|ban|regulat|privacy|ai|var|olympics|rights?)\b/i.test(combined)) {
    score += 1;
  }
...
function isControversialNewsItem(item) {
  return controversyScoreFromText(item?.sourceTitle, item?.sourceDescription, item?.category) >= MIN_CONTROVERSY_SCORE;
}
...
function filterNewsItems(items, options = {}) {
  ...
  return items
    ...
    .filter(item => {
      ...
      // Image is NOT a hard requirement — fallback images are assigned in the generator
      ...
      if (!isControversialNewsItem(item)) return false;
...
module.exports = {
  controversyScoreFromText,
  filterNewsItems,
  fingerprintTitle,
  hasUsablePreviewImage,
  isControversialNewsItem,
};
